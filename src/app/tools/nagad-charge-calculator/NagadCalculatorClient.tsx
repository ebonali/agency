...
import { Smartphone, Building2, UserCircle2, Zap } from "lucide-react";
import { motion } from "framer-motion";

export default function NagadCalculatorClient() {
  const [amount, setAmount] = useState<string>("1000");
  const [method, setMethod] = useState<"app" | "ussd">("app");
  const [accountType, setAccountType] = useState<"regular" | "islamic">("regular");

  // Nagad official cash out rates (including VAT)
  // App: 12.99 tk per 1000 (1.299% approx 1.30%)
  // USSD: 15.00 tk per 1000 (1.50%)
  const rates = {
    app: { regular: 0.01299, islamic: 0.01299 },
    ussd: { regular: 0.015, islamic: 0.015 },
  };
  ...
  const calculateResults = () => {
    ...
    const rate = rates[method][accountType];
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch mb-16">
      ...
      >
        <div className="absolute -inset-0.5 bg-gradient-to-r from-orange-600/20 to-orange-900/20 rounded-2xl blur opacity-30 group-hover:opacity-100 transition duration-1000 group-hover:duration-200"></div>
          ...
          <div className="space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 p-1 rounded-xl bg-zinc-950/50 border border-white/5 shadow-inner">
                  {(["regular", "islamic"] as const).map((p) => (
                    <button
                      key={p}
                      onClick={() => setAccountType(p)}
                      className={`relative px-4 py-3 rounded-lg font-bold text-xs uppercase tracking-widest transition-all ${
                        accountType === p
                          ? "bg-white text-black shadow-[0_0_20px_rgba(255,255,255,0.1)]"
                          : "text-zinc-500 hover:text-zinc-300 hover:bg-white/5"
      ...
      >
        ...
        >
          ...
          <p className="text-xs text-zinc-400 leading-relaxed font-medium">
            Always use the <strong className="text-white">Nagad App</strong> to enjoy the lowest cash out rate of <strong className="text-white">৳12.99 per 1000 (1.30%)</strong>.
          </p>
        </motion.div>
      </motion.div>
    </div>
